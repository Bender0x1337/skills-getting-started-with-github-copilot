document.addEventListener("DOMContentLoaded", () => {
                ...
                .map(
                  (email) => `
                    <li class="participant-pill" data-email="${email}" data-activity="${name}">
                      <span>${email}</span>
                      <button type="button" class="remove-participant" aria-label="Remove ${email}" title="Unregister ${email}">×</button>
                    </li>
                  `
                )
                .join("")
  ...
  activitiesList.addEventListener("click", (event) => {
    ...
    const participantItem = removeButton.closest(".participant-pill");
    if (!participantItem) {
      return;
    }
